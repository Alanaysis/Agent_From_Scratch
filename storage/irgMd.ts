 ...
 */
export async function readIrgMd(cwd: string): Promise<string> {
  const parts: string[] = [];
 ...
 */
export function parseDocRefs(content: string): string[] {
  const refs: string[] = [];
  const regex = /@doc:([\w-]+)/g;
 ...
 */
export async function addDocRef(cwd: string, docId: string): Promise<void> {
 ...
 */
export async function removeDocRef(cwd: string, docId: string): Promise<void> {
  const filePath = join(cwd, IRG_MD_FILENAME);
  let content: string;
 ...
 */
export async function isDocInjected(cwd: string, docId: string): Promise<boolean> {
  try {
 ...
 */
export async function getFullInjectionContent(cwd: string): Promise<string> {
