...
import type { WorkflowStep } from "./workflowIndex";
...
function getTemplatePath(cwd: string, templateId: string): string {
  ...
  return all
    .map((t) => {
      ...
      for (const word of words) {
        ...
        if (t.tags.some((tag) => tag.toLowerCase().includes(word))) score += 1;
      }

      // Exact name match bonus
      if (t.name.toLowerCase() === lowerQuery) score += 10;
...
export async function saveTemplate(
  ...
  await writeFile(
    getTemplatePath(cwd, template.id),
...
export function formatTemplatesForPrompt(templates: WorkflowTemplate[]): string {
  ...
  for (const t of templates) {
    ...
    if (t.useCase) parts.push(`Use when: ${t.useCase}`);
