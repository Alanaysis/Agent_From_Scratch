...
export async function saveMemory(cwd: string, content: string): Promise<void> {
  ...
  const truncated =
    content.length > MEMORY_CHAR_LIMIT
      ? content.slice(0, MEMORY_CHAR_LIMIT)
      : content;
  await writeFile(getMemoryPath(cwd), truncated, "utf8");
}

export async function rebuildMemoryFromKnowledge(cwd: string): Promise<string> {
...
export async function getMemoryForSystemPrompt(cwd: string): Promise<string> {
