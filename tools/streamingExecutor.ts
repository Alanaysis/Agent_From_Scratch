...
import { executeBeforeHooks, executeAfterHooks, type HookContext } from "./hooks";
...
export async function executeWithInstrumentation<Input, Output>(
  ...
  const input = beforeResult.modifiedInput !== undefined
    ? (beforeResult.modifiedInput as Input)
    : originalInput;
  ...
  try {
    ...
    if (afterResult.replaceResult && afterResult.newResult !== undefined) {
      result = { ...result, data: afterResult.newResult as Output };
    }
