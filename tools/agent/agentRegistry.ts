...
export const BUILTIN_AGENTS: Record<string, AgentDefinition> = {
  ...
  pm: {
    name: "pm",
    description:
      "Project Manager agent that decomposes goals into task plans using templates and documents",
    systemPrompt: [
  ...
  "grpc-worker": {
    ...
    systemPrompt: [
      ...
      "",
      "```",
      "GrpcClient(protoFile=\"protos/AlgoService.proto\", service=\"AlgoGRPC.AlgoService\", method=\"SendMessage\", address=\"<address from task>\", payload={...})",
      "```",
      "",
      "IMPORTANT: Always use the EXACT address and payload provided in the task's '## gRPC Task' section. Do NOT substitute a default or example address.",
