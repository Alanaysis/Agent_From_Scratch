...
export type TeamMessage = {
  from: string;
  to: string | "all";
  type: "task_assign" | "task_result" | "question" | "answer" | "status_update";
  content: string;
  timestamp: number;
};
...
export function getTeamDefinition(teamName?: string): TeamDefinition | null {
  if (!teamName?.trim()) return null;
  return BUILTIN_TEAMS[teamName.toLowerCase()] ?? null;
}

export function getToolDefsForTeamMember(
