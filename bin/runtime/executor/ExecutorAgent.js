...
import { updateTaskInfo, readTaskInfo, getUnblockedTasks } from "../../storage/taskIndex";
...
async function getDependencyResults(task) {
    if (!task.dependsOn || task.dependsOn.length === 0)
        return [];
    const deps = await Promise.all(task.dependsOn.map(async (depId) => {
        try {
...
export class ExecutorAgent extends EventEmitter {
    ...
    constructor(parentContext, config = {}) {
        super();
    ...
    async pollAndExecute() {
        ...
        try {
            ...
            const approvalTasks = unblocked.filter(t => t.requiresApproval && t.assignee && !this.activeTasks.has(t.id) && !this.pendingApprovalTasks.has(t.id));
    ...
    async executeTask(taskId) {
        ...
        try {
            ...
            const updated = await updateTaskInfo(cwd(), taskId, { status: "in_progress", sessionId }, this.config.executorName);
            if (!updated) {
            ...
            await updateTaskInfo(cwd(), taskId, { status: finalStatus }, this.config.executorName);
            ...
            await updateTaskInfo(cwd(), taskId, { status: "failed", lastError: errorMessage }, this.config.executorName);
    ...
    async buildTaskPrompt(task, dependencyResults) {
        ...
        if (task.grpcConfig) {
            ...
            if (task.grpcConfig.metadata)
                prompt += `- metadata: ${JSON.stringify(task.grpcConfig.metadata)}\n`;
            if (task.grpcConfig.deadline)
                prompt += `- deadline: ${task.grpcConfig.deadline}\n`;
    ...
    /** Trigger reflection for knowledge extraction (fire-and-forget) */
    triggerReflection(sessionId, title, allMessages) {
    ...
    async executeTaskNow(taskId) {
        ...
        try {
            ...
            if (isNewSession) {
                ...
                await updateTaskInfo(cwd(), taskId, { sessionId }, this.config.executorName);
                console.log(`[Executor] Updated task ${taskId} with sessionId ${sessionId}`);
            }
            if (task.status !== "in_progress") {
                await updateTaskInfo(cwd(), taskId, { status: "in_progress", assignee: task.assignee || this.config.executorName, sessionId }, this.config.executorName);
            }
            this.emit("taskProgress", taskId, "Agent running...");
            // Re-read task to get the latest data (title, description may have been updated)
            const freshTask = await readTaskInfo(cwd(), taskId);
...
export async function startExecutor(parentContext, config) {
...
export function forceExecuteTask(taskId) {
    if (!globalExecutor) {
