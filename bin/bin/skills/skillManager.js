...
function getUserSkillsDir(cwd) {
    return join(cwd, ".claude-code-lite", "skills");
}
async function ensureSkillsDir(cwd) {
...
export async function createSkill(cwd, name, content, trigger, allowedTools, source = "auto") {
...
export async function patchSkill(cwd, name, patch, changeDescription, source = "auto") {
    ...
    if (patch.allowedTools)
        existing.frontmatter.allowedTools = patch.allowedTools;
    if (patch.description)
        existing.frontmatter.description = patch.description;
...
export async function listUserSkills(cwd) {
...
export async function deprecateSkill(cwd, name, reason) {
...
export async function recordSkillUsage(cwd, name, success) {
    const skill = await readSkill(cwd, name);
    if (!skill)
        return;
...
function parseSkillContent(raw) {
    ...
    return {
        name: fm.name || "",
        trigger: Array.isArray(fm.trigger) ? fm.trigger : [],
        frontmatter: fm,
        content: body,
    };
}
