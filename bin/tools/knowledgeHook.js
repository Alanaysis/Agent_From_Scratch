 ...
 */
export function registerKnowledgeHook(cwd) {
    registerHooks({
...
async function handleError(cwd, ctx, error) {
    ...
    for (const { pattern, insight } of ERROR_PATTERNS) {
        if (pattern.test(errorMessage)) {
            await addKnowledge(cwd, "anti_pattern", `${ctx.toolName}: ${insight} (Error: ${errorMessage.slice(0, 100)})`, "user_implicit", [ctx.toolName.toLowerCase(), "error"], 0.7);
    ...
    if (errorCount >= 3) {
        await addKnowledge(cwd, "anti_pattern", `Tool "${ctx.toolName}" has failed ${errorCount} times in this session. Consider alternative approaches.`, "user_implicit", [ctx.toolName.toLowerCase(), "recurring-error"], 0.8);
...
async function handleSuccess(cwd, ctx) {
    const toolKey = ctx.toolName;
    // Track usage count
    ...
    if (usageCount === 15) {
        await addKnowledge(cwd, "pattern", `Tool "${ctx.toolName}" used 15+ times in session - complex workflow detected`, "user_implicit", [ctx.toolName.toLowerCase(), "efficiency"], 0.6);
        await rebuildMemoryFromKnowledge(cwd).catch(() => { });
    }
}
/** Reset tracking counters (call at session start) */
export function resetKnowledgeHookCounters() {
