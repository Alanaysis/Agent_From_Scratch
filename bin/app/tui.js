...
import { query, executeWorkMap } from "../runtime/query";
...
import { parseSkillToWorkMap, } from "../runtime/workmap";
...
function withSinglePanelBorder(title, lines, width, height) {
    const innerWidth = Math.max(8, width - 2);
    const bodyRows = Math.max(0, height - 2);
    const normalized = [
        `┌${trimTextPlain(` ${title} `, innerWidth).replace(/ /g, "─")}┐`,
...
function withSinglePanelBorderColored(title, lines, width, height, borderColor) {
    const innerWidth = Math.max(8, width - 2);
    const bodyRows = Math.max(0, height - 2);
    const normalized = [
...
function applyModalOverlay(lines, modal, width, height) {
    ...
    if ("phase" in modal) {
        ...
        if (modal.phase === "collecting") {
            ...
            // 确认阶段
            const specLines = Object.entries(modal.collectedValues).map(([key, value]) => `${key}: ${CYAN}${value}${RESET}`);
            const contentLines = [
                "Final Configuration:",
                "",
                ...specLines,
...
function renderScreen(state, runtimeRef) {
    ...
    if (state.streamingAssistantText.trim()) {
        ...
        messageLines.push(...wrapText(`${state.theme === 'dark' ? pc.bgBlack(pc.white('SLI')) + pc.black(` ${streamingText}`) : pc.bgWhite(pc.black('SLI')) + pc.white(` ${streamingText}`)}`, Math.max(20, mainWidth - 4)));
    }
...
async function runSlashCommand(line, options, state, runtimeRef, appStateRef, requestPermission, requestSpec) {
    ...
    if (commandLine.startsWith("workmap")) {
        ...
        if (subCommand === "load" || (!state.workMap && (!subCommand || subCommand === "help"))) {
            ...
            if (skillName) {
                const match = skills.find(s => s.name.toLowerCase().includes(skillName.toLowerCase()) ||
    ...
    if (commandLine.startsWith("resume")) {
        ...
        const target = !rawTarget || rawTarget === "latest"
            ? sessions[0]?.id
            : rawTarget === "failed"
...
function autoCompleteSlashCommand(input) {
        ...
        if (subCommand === "load") {
            ...
            const skillMatches = skills
                ...
                return (s.name.toLowerCase().includes(searchTerm) ||
                    s.frontmatter.trigger?.some((t) => t.toLowerCase().includes(searchTerm)) ||
                    searchTerm === "");
            })
    ...
    const matches = helpMessagesAll.filter(msg => msg.toLowerCase().includes(command));
...
export async function startTui(options) {
    ...
    const requestSpec = async (request) => {
        ...
        addActivityStep(state, `configure ${request.skillId}`, "permission", "info");
        renderScreen(state, runtimeRef);
        return new Promise((resolve) => {
            state.modal = {
                ...
                resolve: (spec) => {
                    ...
                    addActivityStep(state, `configured ${request.skillId}`, "permission", "done");
                    resolve(spec);
                },
            };
            renderScreen(state, runtimeRef);
    ...
    const submitPrompt = async (line) => {
        ...
        try {
            if (trimmed.startsWith("/")) {
                ...
                for await (const message of query({
                    ...
                    if (message.type === "tool_result") {
                        ...
                        const durationMs = state.activityStartedAt === null
                            ? undefined
            ...
            addActivityStep(state, interrupted
                ? "interrupted current turn"
                : `error ${message.slice(0, 24)}`, interrupted ? "session" : "error", "failed", durationMs);
        }
        finally {
            state.busy = false;
            state.activityStartedAt = null;
