...
export function createContext(cwd, session, appStateRef, abortController) {
    return {
        cwd,
        abortController,
...
export async function resolveResumeTarget(cwd, raw) {
    if (!raw) {
        return undefined;
    }
    if (raw === "latest") {
...
export async function startRepl(options, { stdin = processStdin, stdout = processStdout } = {}) {
            ...
            try {
                ...
                for await (const message of query({
                    ...
                    if (message.type === "tool_result") {
                        ...
                        if (message.isError) {
                            stdout.write(`[tool:error] ${summary} · ${summarizeUnknown(message.content, 80)}\n`);
                        }
                        else {
                            stdout.write(`[tool:done] ${summary} · ${summarizeUnknown(message.content, 80)}\n`);
                        }
