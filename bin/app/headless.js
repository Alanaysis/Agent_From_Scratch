...
import { deleteSessionInfo, getSessionInfoFilePath, listSessions, readSessionInfo, } from "../storage/sessionIndex";
import { deleteTranscript, getTranscriptPath, readTranscriptMessages, } from "../storage/transcript";
...
export function formatSessionList(sessions, options) {
    ...
    const lines = sessions
        ...
        const count = session.messageCount !== undefined
            ...
            : "";
        const stats = session.toolUseCount !== undefined || session.errorCount !== undefined
...
export function formatSessionMetadata(session) {
    return [
        `session: ${session.id}`,
...
export function formatInspectView(cwd, session, messages, recentCount = 8) {
    ...
    return [
        ...
        recentErrors.length > 0
            ? recentErrors
                .map((message, index) => `${index + 1}. ${summarizeUnknown(message.content, 200)}`)
                .join("\n")
            : "(none)",
        "",
...
export function formatCleanupSummary(removed, skippedCount, dryRun = false) {
    if (removed.length === 0) {
...
export function formatMarkdownExport(session, messages) {
    return [
        `# Session ${session.id}`,
        "",
        "## Metadata",
...
export function formatJsonExport(session, messages) {
...
export function formatTranscriptEntry(message) {
    if (message.type === "user") {
...
export function formatTranscriptMessages(messages, compact = false) {
...
export async function resolveSessionIdArg(cwd, rawSession) {
    if (!rawSession) {
        return undefined;
    }
...
export async function resolveSessionIdForChat(cwd, rawSession) {
    return resolveSessionIdArg(cwd, rawSession);
}
export async function parseChatCommandOptions(cwd, args) {
    let sessionRef;
    const promptParts = [];
...
export async function parseExportCommandOptions(cwd, args) {
...
export function buildSyntheticAssistant(toolName, toolInput) {
    return {
...
export async function confirmOrThrow(message, autoApprove) {
    if (autoApprove)
        return;
...
export function parseCleanupCommandOptions(args) {
    let keep;
    let olderThanDays;
    let dryRun = false;
    let status;
...
export function parseSessionsCommandOptions(args) {
    let limit;
    let status;
    for (let index = 0; index < args.length; index += 1) {
        const arg = args[index];
...
export async function deleteSessionArtifacts(cwd, sessionId) {
    await Promise.all([
        deleteSessionInfo(cwd, sessionId),
        deleteTranscript(cwd, sessionId),
    ]);
}
export async function confirmWithSessionRule(message, autoApprove, tool, inputValue, context) {
    if (autoApprove)
        return;
    if (!input.isTTY || !output.isTTY) {
        throw new Error(`${message}. Re-run with --yes to auto-approve.`);
...
export function createToolContext(cwd, appStateRef, session, abortController) {
    return {
        cwd,
...
export async function executeCliCommand(cwd, argv, autoApprove = false, hooks) {
    ...
    if (parsed.kind === "utility") {
        ...
        if (parsed.utilityName === "export-session") {
            ...
            const info = (await readSessionInfo(cwd, options.sessionId)) ||
                (await listSessions(cwd)).find((session) => session.id === options.sessionId);
            if (!info) {
        ...
        if (chat.sessionId) {
            session.hydrateMessages(await readTranscriptMessages(cwd, chat.sessionId));
        }
        ...
        for await (const message of query({
            ...
            onPermissionRequest: async (request) => {
                ...
                await confirmWithSessionRule(request.message, autoApprove, tool, request.input, context);
                return true;
            },
        })) {
    ...
    const effectiveInput = permissionDecision.behavior === "allow" && permissionDecision.updatedInput
    ...
    try {
        ...
        if (result.kind === "utility") {
            if (shouldStream && result.utilityName === "chat") {
                const transcriptPath = typeof result.output === "object" &&
                    result.output !== null &&
